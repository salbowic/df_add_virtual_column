...
def test_numeric_constants_in_rules():
    df = pd.DataFrame([[1, 2]] * 2, columns=["label_one", "label_two"])
    df_expected = pd.DataFrame([[1, 2, 2]] * 2, columns=["label_one", "label_two", "label_three"])
    df_result = add_virtual_column(df, "label_one * 2", "label_three")
    assert df_result.equals(df_expected), f"The function should support numeric values in rules."


def test_unsupported_operation_division():
    df = pd.DataFrame([[4, 2]] * 2, columns=["label_one", "label_two"])
    df_result = add_virtual_column(df, "label_one / label_two", "label_three")
    assert df_result.empty, f"The function should return an empty dataframe for unsupported operations like division."


def test_empty_result_when_invalid_existing_columns():
    df = pd.DataFrame([[1, 2]] * 2, columns=["label-one", "label_two"])
    df_result = add_virtual_column(df, "label_two * 2", "label_three")
    assert df_result.empty, f"Should return an empty df when existing df contains invalid columns."
